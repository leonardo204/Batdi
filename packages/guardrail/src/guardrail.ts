/**
 * @batdi/guardrail — rule-based 입력/출력 가드레일 (순수 함수, ADR-051)
 *
 * SSOT: Ref-docs/specs/design/batdi-persona-guardrail.md §6.2
 *   B. 일베 밈/혐오, C. 프롬프트 해킹, D. 비속어/비하/위협/도박/자해
 *
 * 보안 가드레일 룰을 복제(drift=우회 위험)하지 않기 위해 본 공유 패키지로 추출한다
 * (단일 SSOT). agent(InputGuardrail 노드)와 api(custom_persona 저장 전 검증)가
 * 동일 함수를 import 한다.
 *
 * 매칭은 반드시 정규화본(`toNormalizedForm` 결과) 기준으로 수행한다(원문 금지).
 ...
 * @param normalized toNormalizedForm 결과(userMessageNormalized)
 */
