...
> L1 authoring 표현(도메인 widget + `{{bind:}}` placeholder). DataBinder가 emit 시 **A2UI 표준 3-op**(`createSurface`/`updateComponents`/`updateDataModel`, `component` 키, `{path:}` 값슬롯)로 컴파일한다 — 정식 와이어 포맷·골든 샘플은 [a2ui-palette-schema](../interface/batdi-a2ui-palette-schema.md)·[agui-contract §2.2.1](../interface/batdi-agui-contract.md).

```text
(authoring) LLM 선택: scoreboardWidget · pitchingLineWidget · text
            각 값 슬롯 = {{bind:"data.home.score"}} placeholder, 리액션 = {{llm.reaction}}
(emit)      createSurface → updateComponents(component 트리) → updateDataModel{value: DB값}
```

LLM은 `scoreboardWidget`·`pitchingLineWidget`·`text`를 **선택**했을 뿐, 실제 수치는 DataBinder가 DB에서 가져와 `updateDataModel`로 주입한다(환각 차단, ADR-009/019).
