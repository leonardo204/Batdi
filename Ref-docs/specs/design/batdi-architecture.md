...
| ADR-058 | **NewsGraph v2 — Gemini Google Search grounding 실시간 뉴스 + cache_news TTL(RSS 크롤 대체)** | **P4(2026-06-19) 구현.** ADR-048 RSS 배치 크롤을 **Gemini grounding 실시간 검색**으로 교체(빌링 활성·유료티어). **비용**: 2.5 grounding 일 1,500건 무료(초과 $35/1k) → MVP 무료 범위 + cache_news 캐싱으로 호출 절감. ① **NewsGraph v2**(agent services/news-search.ts): `searchNews(query)` — `new ChatGoogleGenerativeAI({model:'gemini-2.5-flash'}).bindTools([{ googleSearch:{} }])`(2.5 grounding 툴; google_search_retrieval 은 1.5용) invoke("KBO {query} 최신 뉴스 N건 제목·출처·요약") → content + response_metadata.groundingMetadata(출처 URL/제목) 파싱 → rows. 키없음/오류→null(폴백). ② **질의 추출**: userMessage 에서 뉴스 토픽 추출("한화 뉴스"→"한화 이글스", "오타니 뉴스"→"오타니", 토픽 없으면 team 기본) — **팀 고정 아닌 임의 질의 지원**(UX 향상). ③ **캐싱**(제안 플로우): cache_news 에 `queryKey`(정규화 질의 해시) 컬럼 추가·마이그레이션. fetchNewsData(query): cache_news(queryKey·expiresAt>now) HIT→DB, MISS/stale→searchNews→cache_news 저장(queryKey·source·TTL ~30분)→rows. ④ **출처 표기**: grounding 메타데이터 URL 을 라인/카드에 포함(환각 방지·신뢰성). news_compact 템플릿 재활용(rows.N.line). ⑤ **RSS 크롤 deprecate**: api DailyNewsScheduler/NewsRssScraper 는 비활성(NEWS_CRAWLER_ENABLED 기본 off 유지) — grounding 이 request-time 대체. **비결정성**: L0 캐시 불가(질의 기반 cache_news TTL 로 대체). **잔여**: grounding 응답 파싱 안정화, 출처 클릭 UI, 질의 추출 정교화. |
| ADR-057 | **팀 상대전적(h2h) — KBO TeamRank 매트릭스 크롤 + 신규 h2h intent + HeadToHeadGraph + 카드** | **P4(2026-06-19) 구현.** 미보유였던 상대전적을 합법 크롤+신규 intent 풀스택으로. **소스**: koreabaseball.com `/Record/TeamRank/TeamRank.aspx`(robots 허용·이미 standings 크롤 중인 페이지)에 **팀간 상대전적 매트릭스 표** 존재(정찰 확정: 행=팀, 열=상대팀 "(승-패-무)", 셀 "5-3-0", ■=자기, 합계 컬럼). 봇 우회 없음. ① **스키마**: `team_head_to_head`(season, teamId, opponentId, opponentName, wins, losses, draws, `@@unique([season,teamId,opponentId])`) + 마이그레이션. ② **크롤**: scrapeTeamRank 와 동일 페이지에서 h2h 매트릭스 표 추가 파싱(헤더 "(승-패-무)"/■ 대각선으로 식별), 한글 팀명→teamId 매핑, W-L-D 분해 → 행. crawler-health 'h2h' 소스, 스케줄러 배선. ③ **intent**: packages/types Intent += `'h2h'`, intent-router 규칙(`상대전적|맞대결|천적|상대 전적`). ④ **HeadToHeadGraph**(agent services/head-to-head-graph.ts): fetchHeadToHead(teamId) → team_head_to_head 조회 → `{rows:[{line:"vs {상대} {W}승{L}패{D}무"}]}`(승률/순 정렬). ⑤ **템플릿**: 신규 `h2h_compact`(title "상대전적" + rows.N.line, standings 패턴) + registry/TEMPLATE_CATALOG/TEMPLATE_BY_INTENT.h2h. ⑥ **배선**: state.headToHeadData + service-data h2h 분기 + emit h2h(데이터 카드/없으면 폴백·L0 미write). **MVP**: 팀 단위 상대전적. **잔여**: 선수 vs 선수 맞대결(KBO 공식 미제공), 특정 두 팀 지정 질의("한화 vs 두산")→head_to_head_widget 2-entity 매핑. |
| ADR-056 | **LineupCrawler — KBO 공식 GameCenter 선발투수 크롤(robots 허용·정형 속성, LineupGraph 실데이터)** | **P4(2026-06-18) 구현.** ADR-052 의 LineupGraph 데이터 공백을 **합법 크롤**로 마감. **소스 적법성**: koreabaseball.com robots.txt 는 `/Common//Help//Member//ws/` 만 Disallow → **GameCenter(`/Schedule/GameCenter/Main.aspx`)는 허용**(이미 일정 크롤 중인 도메인, **봇차단 우회 일절 없음**·기존 10초·동시1·healthScore 규칙 준수). **정찰 확정**: `.today-game .game-cont` LI 가 정형 데이터 속성 보유 — `g_id`(=gameKey 패턴 YYYYMMDD+팀코드), `g_dt`, `s_nm`(구장), `home_nm`/`away_nm`(한글 팀명), `home_id`/`away_id`(팀코드 OB/KT 등), `home_p_id`/`away_p_id`(선발투수 ID), `start_ck`/`lineup_ck`(라인업 가용 플래그). 선발투수 **이름**은 텍스트("선{원정}P VS 선{홈}P")라 텍스트+속성 병행 파싱. ① **스키마**: `game_lineups`(gameKey unique, gameDate, homeTeamId/awayTeamId[한글명→teamId 매핑·4팀만], homeTeamName/awayTeamName, homeStarter/awayStarter, stadium, gameTime, status) + 마이그레이션. ② **크롤러**(kbo-scraper.scrapeLineups 또는 신규): GameCenter Playwright → game-cont 속성+텍스트 파싱 → 행. CrawlerHealthManager 'lineup' 소스 추가. 스케줄러 배선(경기 임박 갱신 위해 daily+추가 스윕 best-effort). ③ **LineupGraph 실데이터화**: fetchLineupData(teamId) → game_lineups 에서 팀 당일/차기 경기 → lineup_compact 데이터(team + 선발 매치업·구장·시각 행). **MVP**: 선발투수 매치업(항상 가용). 풀 타순(9명)은 lineup_ck 게시 시점 상세페이지 필요 → 잔여. **잔여**: 풀 타순 크롤, 팀코드 전체 매핑(현 4팀), 상대전적은 TeamVsTeam(302 리다이렉트) 별도 조사. |
| ADR-055 | **P4-W11 로컬 Web Push — VAPID + PushProvider 추상화 + SW + 푸시 트리거 4종** | **P4(2026-06-18) 구현.** PWA Web Push(11.1~11.3). **PushProvider 추상화 필수**(CLAUDE.md — P6 FCM 어댑터 교체 대비): `PushProvider` 인터페이스(send/sendToUser) + `LocalWebPushProvider`(web-push npm·VAPID). ① **스키마**: `push_subscriptions`(userId FK, endpoint unique, p256dh, auth, createdAt) + 마이그레이션. ② **api push 모듈**: POST /api/push/subscribe(구독 저장)·/unsubscribe + `PushService.sendToUser(userId, payload)`(구독 조회→provider.send, 만료 410 구독 정리). VAPID 공개키 GET /api/push/vapid-public-key(env VAPID_PUBLIC_KEY, 미설정 시 비활성 graceful). ③ **푸시 트리거 4종**(push-triggers.ts 순수 결정 함수 + 유닛테스트): 경기 시작 30분 전(kbo_games date diff)·역전/동점(직전 score 스냅샷 diff)·관심선수 활약(user_favorites∩스탯 이벤트)·**레벨업**(ADR-049 updateLevelProgress.leveledUp). 결정 로직 단위테스트가 DoD. ④ **web**: Service Worker(public/sw.js — push/notificationclick) + 구독 플로우(Notification.requestPermission + PushManager.subscribe[VAPID applicationServerKey] → POST subscribe), /settings 또는 훅에서 알림 켜기. **MVP/잔여**: 실시간 이벤트 배선(역전/동점 score 폴링·관심선수 활약 감지)은 크롤/스케줄 통합 필요 — 트리거 결정 함수+레벨업 경로 우선, 경기시작 30분전은 cron 스윕 best-effort. iOS 16.4+ 홈화면 추가 안내(P6 실 VAPID 배포). NEXT_PUBLIC_VAPID 키 로컬 생성. |
| ADR-054 | **경기 예측(MatchPrediction) — 승부 예측 + 적중률(Lv2 해금 기능)** | **P4(2026-06-18) 구현.** platform-ops §13 Lv2 해금 "경기 예측"을 실 기능으로. 미구현이던 예측 기능 신설. ① **스키마**: `match_predictions`(id, userId FK, gameKey, predictedWinner[home|away], createdAt, `@@unique([userId,gameKey])` 멱등) + 마이그레이션. ② **api predictions 모듈**: POST /api/predictions(JwtAuthGuard, **requireLevel(2)** — Lv2 해금) {gameKey, predictedWinner} — kbo_games 존재+미종료 검증, 화이트리스트, upsert(예측 변경 허용). GET /api/predictions/me — 내 예측 목록 + **적중률 산정**(kbo_games join: FINISHED 경기의 home/away score 로 실제 승자 도출 → predictedWinner 일치=correct, **on-read 계산**[저장 안 함·멱등], accuracy=correct/finished). ③ **web /my/predictions 페이지**: 예정 경기(schedule 소스)에 홈/원정 승 예측 버튼 + 과거 예측 적중 여부·누적 적중률 카드. Lv2 미만 403 locked 안내. /my 네비 탭 추가. ④ **적중률→레벨**: Lv5 고급 게이트(적중률 55%+)의 데이터 소스 마련(ADR-049 잔여 일부 해소). **MVP 범위**: 예측 마감(경기 시작 시각)·점수 정확도 예측·예측 통계 상세는 후속. **잔여**: 예측 마감 시각 게이팅, 적중 시 XP 보너스, Lv5 적중률 게이트 실배선. |
| ADR-053 | **레벨 해금 게이팅(LevelGuard) + 커스텀 닉네임(Lv5) — platform-ops §13 해금을 실 제한으로** | **P4(2026-06-18) 구현.** Lv 해금이 /my/level 레이블만이고 실 게이팅이 없던 것을 실제 제한으로 구현. ① **LevelGuard 유틸**(api): `requireLevel(userLevel, min)` → 미달 시 ForbiddenException `{locked:true, requiredLevel, levelName}`(level-rules.ts 이름 재사용). ② **게이팅 적용**: Lv4=상세통계·선수비교 → showPlayerDetail(GET players/:id)·showTeamComparison(GET stats/compare) 엔드포인트가 user.level≥4 요구(미달 403 locked). Lv5=커스텀 닉네임 → **PATCH /users/me/nickname**(신규, level≥5, displayName 갱신·길이/가드레일 검증) + /settings 닉네임 필드(Lv5 미만은 비활성+해금 안내). ③ 프론트 액션 handler 는 403 locked 응답 시 "Lv4에서 해금돼요" 안내(오버레이/토스트). **MVP 범위**: 두 번째 페르소나 스타일(Lv3)·숨겨진 페르소나(Lv5)는 personaStyle 옵션 게이팅으로 후속(잔여). user.level 은 ADR-049 훅이 채움. **잔여**: 페르소나 스타일 레벨 게이팅, 해금 시 축하 연출. |
| ADR-052 | **ScheduleGraph + LineupGraph — schedule/lineup intent 서브그래프 배선(NewsGraph 패턴)** | **P4(2026-06-18) 구현.** ADR-047 에서 데이터 부재로 미배선이던 schedule/lineup intent 를 실 서브그래프로 채운다(기능 갭 마감). ① **ScheduleGraph**(services/schedule-graph.ts): `fetchScheduleData(teamId)` getPrisma best-effort — **kbo_games 재사용**(score-graph 와 동일 소스), 오늘 이후(date>=today) SCHEDULED 경기를 팀 필터+날짜 asc take 5 → `{rows:[{line}]}`, formatScheduleLine="M/D(요일) 홈 vs 원정 · 구장/시각"(사전 포맷, 수치 LLM 미생성). schedule_compact 템플릿(8.4) 데이터. ② **LineupGraph**(services/lineup-graph.ts): **라인업 크롤 데이터 미보유** → 현재는 graceful 처리(getPrisma 로 가능한 선발 정보 조회 시도, 없으면 null→emit 이 "라인업은 경기 임박 시 공개돼요" 팀톤 폴백). lineup_compact 템플릿은 데이터 들어오면 즉시 사용. **잔여: 선발/타순 크롤러(신규 소스) 도입 후 실데이터**. ③ **배선**: service-data 에 `intent==='schedule'`→fetchScheduleData, `intent==='lineup'`→fetchLineupData 분기 + state.scheduleData/lineupData 채널 + emit 분기(데이터 있으면 템플릿 카드/없으면 텍스트 폴백·L0 미write) + TEMPLATE_BY_INTENT.schedule=schedule_compact·lineup=lineup_compact 배선(ADR-047 미배선 해제). intent-router 규칙·템플릿은 기존 보유. **잔여**: lineup 실데이터 크롤러, schedule 더블헤더/우천취소 표기. |
| ADR-051 | **P4-W10 10.5 커스텀 페르소나 편집기 + 저장 가드레일 — @batdi/guardrail 공유 패키지(agent·api 공용·drift 0)** | **P4(2026-06-18) 구현.** /settings 에서 사용자가 기본 프롬프트 위에 자기 지시(500자, user_instruction priority=2)를 추가하고, **저장 전 일베+프롬프트해킹+비속어 가드레일로 차단**한다(persona-guardrail §313/421). **핵심 결정**: 가드레일은 보안 자산이라 api 복제 시 drift = 우회 위험 → agent 의 순수 함수 `checkInputGuardrail/checkOutputGuardrail`(patterns)+`toNormalizedForm`(NFKC+자모+homoglyph+이모지 제거)을 **신규 `packages/guardrail`(@batdi/guardrail)** 로 추출해 agent(input-guardrail/normalizer 노드는 thin wrapper·re-export 로 기존 import 호환)·api(페르소나 저장) 가 **단일 구현 공용**. ① **공유 패키지**: normalize.ts(toNormalizedForm+homoglyph 맵)+guardrail.ts(일베/프롬프트해킹/비속어 룰 + check 함수, @batdi/types GuardrailResult). 기존 normalizer/input-guardrail 테스트는 re-export 로 그대로 통과(회귀 0). ② **저장 API**: POST /api/users/me/persona(JwtAuthGuard) {customPersona} — 길이≤500(BadRequest) → toNormalizedForm → checkInputGuardrail → **!pass 면 400 차단**(type 반환, 저장 안 함) → pass 면 personal_agent_state.customPersona upsert. GET /api/users/me/persona(현재값 로드). ③ **/settings 페이지**: 페르소나 textarea(500자 카운터)+저장, 가드레일 거부 시 에러 표시("일베/부적절 표현 저장 불가"), tokens.css 스타일, 인증가드(/my 패턴). **openPersonaEditor 액션(10.1)의 타겟**. ④ custom_persona 는 prompt-builder 가 `<custom_persona>`(priority=2) 로 주입 + isPersonalized→L0 캐시 write 금지(기존). **잔여**: SemanticGuardrail(LLM 의미 검증) 저장 적용, 알림/보존기간/학습동의 등 /settings 나머지 섹션, 커스텀 닉네임(Lv5 해금). |
| ADR-050 | **P4-W10 10.1/10.2 useCopilotAction LLM 툴콜 e2e — 프론트 액션→그래프 state.tools→bindTools→tool_call→핸들러** | **P4(2026-06-18) 설계·구현.** "LLM이 프론트 함수 호출"을 결정론 그래프와 공존시키는 경로를 **역공학으로 확정**(설치본 실측). **메커니즘**(@copilotkit 1.60·@ag-ui/langgraph 0.0.41·@langchain/langgraph 1.4.1): ① useCopilotAction 등록 액션은 POST /copilotkit body 의 **`tools`**(name+description+parameters만, handler 는 클라이언트 `_tools` 레지스트리 잔류·신원 forwardedProps 와 별개 필드). ② @ag-ui/langgraph `langGraphDefaultMergeState` 가 tools 를 LangGraph SDK runs.stream 의 **`input`(그래프 state)** 으로 병합 → 노드는 `state.tools`(base 보장)·`state.copilotkit.actions`(runtime CopilotKit 서브클래스)·`state["ag-ui"].tools` 로 읽음. **신원(config.configurable, ADR-043)과 다른 채널**. ⚠️ **CoreStateAnnotation 에 passthrough 채널 미선언 시 input 키가 노드 전에 드롭**됨 → state.ts 에 `tools`/`copilotkit` lastValue 채널 추가. ③ chat 노드(generateChatReply)가 frontendTools 있으면 `model.bindTools(액션→tool 변환)` 로 LLM 주입 — **액션 요청은 키워드 미매칭이라 chat intent(catch-all)로 라우팅**되므로 chat 경로 1곳 확장이면 충분(결정론 라우팅 불변·IntentRouter LLM 미사용 유지). ④ LLM 이 tool_calls 를 가진 **AIMessage 일반 방출**(interrupt 불필요) → state.messages 로 스트림 → @ag-ui/langgraph 가 `TOOL_CALL_START/ARGS/END` AG-UI 이벤트 변환 → CopilotKit 클라이언트 `getTool(name).handler` 실행 + 결과 `role:tool` 메시지로 followUp 자동 재호출(라운드트립). ⑤ **A2UI 공존**: a2ui-middleware 는 `a2uiToolNames`(render_a2ui) 만 가로채고 나머지 tool-call 은 `t.next` 통과 → **액션명이 render_a2ui/log_a2ui_event 와 안 겹치면 결정론 카드 방출과 안전 공존**. **10.1 액션 7종**(copilot-actions 스펙) 각 **백엔드 검증 API 1:1**(JwtAuthGuard+소유자/파라미터 검증, LLM 악용 방지): registerFavoritePlayer(POST favorites, user_favorites upsert+Player FK 검증)·toggleNotification(user.settings)·showPlayerDetail(GET player)·requestScoreRefresh(score 캐시 무효화)·showTeamComparison(GET compare)·jumpToConversation/openPersonaEditor(프론트 네비/모달, 검증 불요 read). **10.2**: 각 검증 엔드포인트가 `ToolCallLog`(action_name/params/result/duration_ms) 기록 + LangfuseService **best-effort**(@langfuse 미설치 → 키 있을 때만 thin 래퍼, 실패 무시·레이턴시 0). **✅ e2e 실증(2026-06-18, registerFavoritePlayer 수직 슬라이스)**: 풀스택 헤드리스(Playwright)로 "선수 ID 87번 관심선수 등록" → ① state.tools 채널 생존(노드 진단: `stateToolsKeys=["tools","copilotkit"]`, frontendActions=1) ② Gemini bindTools 가 tool_call 1건 방출(toolCalls=["registerFavoritePlayer"]) ③ **핵심 정정**: `manually_emit_tool_call` 커스텀 이벤트는 **A2UI 미들웨어 surface 빌드 전용**이라 프론트 액션을 트리거 못함(SSE 에 TOOL_CALL 흘러도 핸들러 미발화 실측). **정답 경로 = chat 노드가 구조화된 `tool_calls` 를 가진 AIMessage 를 state.messages 에 실음 → 메시지 스냅샷의 message.toolCalls 를 CopilotKit core `processAgentResult` 가 읽어 getTool(name).handler 실행**(generateChatReply 반환을 string→`{text,toolCalls}` 로 변경, emit-a2ui 가 toolCalls→AIMessage.tool_calls 구성). ④ 클라 handler→`POST /api/favorites/register`(쿠키 신원)→`user_favorites`(87,explicit)+`tool_call_logs`(action/params/result) 기록 확인. render_a2ui ack 카드와 공존(이름 분리). vitest 600/600, typecheck 6/6. **잔여**: 나머지 6 액션+검증 API 확장(증명된 패턴), 액션 트리거가 다른 intent(stats 등)로 라우팅될 때 해당 노드 tool 바인딩, Langfuse SDK 정식 연동(전역 트레이스), showPlayerDetail/showTeamComparison 오버레이 UI, 10.4 /my 페이지. |
| ADR-049 | **P4-W10 10.3 LevelAgent — 5단계 레벨 규칙 + XP write-through 훅(turns 기반, 멱등 recompute)** | **P4(2026-06-18) 구현.** platform-ops §13 의 5단계 레벨 시스템(Lv1 신입팬/Lv2 내야석 대화50회/Lv3 응원단석 200+예측5/Lv4 시즌권 500+연속7일/Lv5 12번째선수 1000+적중55%)을 도입. **MVP 설계 결정**: 예측·연속일·적중률은 미구현 기능이라 **MVP 는 대화 횟수(turns) 기반 XP 단일 게이트**로 레벨을 산정한다(고급 AND-조건은 보류 — 기능 도입 시 합산). turns = `message_count/2`(user+assistant=2/턴, write-through SSOT). `XP_PER_TURN=10` → XP 임계 `[0,500,2000,5000,10000]`(= turns 0/50/200/500/1000). ① **순수 규칙**(personal/level-agent.ts): `LEVEL_RULES`(Lv·이름·minXp·해금) + `computeLevel(xp)`→Lv + `buildLevelProgress(level,xp)`→{currentLevel(이름), bar(사전포맷 `███░░ 60%`), xp(`1200 / 2000 XP`)} (levelProgressWidget 8.3 의 `level.currentLevel/bar/xp` 슬롯 데이터, **수치는 정적 계산·LLM 무관**). ② **XP write-through 훅**(conversation-store updateLevelProgress, persistTurnNode 에서 bumpMessageCount 뒤 호출, best-effort throw 금지): message_count(turns) 읽어 xp=turns×10·level=computeLevel(xp) **멱등 recompute** → users.xp_points/level update(증분 아님 — 소스 of truth=message_count 라 드리프트/중복 0). **레벨업 감지**(prevLevel<newLevel) 반환(11.2 레벨업 푸시 트리거용). ③ deriveKnowledgeLevel(user.level→beginner/core/expert)는 9.5 적응UI 가 이미 소비 — 본 훅이 user.level 을 채우면 레벨 상승이 knowledgeLevel·레벨 카드에 자동 반영. **스키마 변경 없음**(users.level/xp_points 기존 필드). **잔여**: 예측/연속일/적중률 지표 도입 후 고급 게이트 합산, levelProgressWidget→intent 라우팅(/my/level 10.4·"내 레벨" 질의), 레벨업 토스트/푸시(11.2), 세션종료 시점 배치 산정 옵션(현재 매 턴 멱등). |
